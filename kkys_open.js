...
async function home(filter) {
  ...
  let data = JSON.parse(
    await request(url + '/api.php/Index/getTopVideoCategory')
  ).data
  let classes = []
  let filterObj = {}
  for (const type of data) {
    ...
    try {
      ...
      let filterData = JSON.parse(
        await request(url + '/api.php/Video/getFilterType', { type: typeId })
      ).data
...
async function homeVod() {
  let jsonArray = JSON.parse(
    await request(url + '/api.php/Index/getHomePage', { type: 1, p: 1 })
  ).data.video
  let videos = []
...
async function detail(id) {
  let data = JSON.parse(
    await request(url + '/api.php/Video/getVideoInfo', { video_id: id })
  ).data.video
  let vod = {
    vod_id: data.vod_id,
...
async function play(flag, id, flags) {
  try {
    ...
    if (id.indexOf('jqq-') >= 0) {
      ...
      var ids = id.split('-')
      var jxJqq = await req(
        'https://api.juquanquanapp.com/app/drama/detail?dramaId=' +
          ids[1] +
          '&episodeSid=' +
          ids[2] +
          '&quality=LD',
...
async function search(wd, quick) {
  let data = JSON.parse(
    await request(url + '/api.php/Search/getSearch', {
      key: wd,
      type_id: 0,
      p: 1,
    })
