    ...
    try {
      ...
      if (_.isEmpty(filterAll)) {
        let filterData = JSON.parse(
          await request(
            url + '/api.php/provide/searchFilter?type_id=0&pagenum=1&pagesize=1'
          )
        ).data.conditions
...
async function homeVod() {
  let data = JSON.parse(
    await request(url + '/api.php/provide/homeBlock?type_id=0')
  ).data
...
async function category(tid, pg, filter, extend) {
  let reqUrl =
    url +
    '/api.php/provide/searchFilter?type_id=' +
    tid +
    '&pagenum=' +
    pg +
    '&pagesize=24&'
  reqUrl += jinja2(
    'year={{ext.year}}&category={{ext.category}}&area={{ext.area}}',
    { ext: extend }
  )
...
async function detail(id) {
  let data = JSON.parse(
    await request(url + '/api.php/provide/videoDetail?ids=' + id)
  ).data
  // console.log(data);
  let vod = {
    vod_id: data.id,
  ...
  let episodes = JSON.parse(
    await request(url + '/api.php/provide/videoPlaylist?ids=' + id)
  ).data.episodes
  let playlist = {}
...
async function play(flag, id, flags) {
  try {
    let data = JSON.parse(
      await request(
        url + '/api.php/provide/parserUrl?url=' + id + '&retryNum=0'
      )
    ).data
...
async function search(wd, quick, pg) {
  ...
  let data = JSON.parse(
    await request(
      url + '/api.php/provide/searchVideo?searchName=' + wd + '&pg=' + page,
      'okhttp/3.12.0'
    )
  )
  let videos = []
...
function randDevice() {
  return {
    ...
    release: '10',
    buildId:
      randStr(3, false).toUpperCase() +
      _.random(11, 99) +
      randStr(1, false).toUpperCase(),
  }
}
