...
async function category(tid, pg, filter, extend) {
  try {
    ...
    let url =
      getCateFilterUrlPrefix(apiUrl) + tid + getCateFilterUrlSuffix(apiUrl)
    ...
    if (jsonArray !== undefined) {
      for (let i = 0; i < jsonArray.length; i++) {
        ...
        const v = {
          ...
          vod_remarks:
            vObj.vod_remarks !== undefined ? vObj.vod_remarks : vObj.state,
        }
        videos.push(v)
      }
    }
...
function printLog(method, key, value) {
  let options = {}
  options.method = 'GET'
  ...
  try {
    ...
    req(url + '出错了!!key的类型为--->' + typeof value, { method: 'get' })
  }
}
...
async function getFinalVideo(flag, parseUrls, url) {
  ...
  for (const parseUrl of parseUrls) {
    ...
    if (
      tryJson !== null &&
      tryJson.hasOwnProperty('url') &&
      tryJson.hasOwnProperty('header')
    ) {
...
function jsonParse(input, json) {
  try {
    ...
    if (
      jsonPlayData.hasOwnProperty('data') &&
      typeof jsonPlayData.data === 'object' &&
      !jsonPlayData.hasOwnProperty('url')
    ) {
    ...
    for (let b = 0; b < vipWebsites.length; b++) {
      if (host.includes(vipWebsites[b])) {
        if (vipWebsites[b] === 'iqiyi.com') {
          // 爱奇艺需要特殊处理
          if (
            url.includes('iqiyi.com/a_') ||
            url.includes('iqiyi.com/w_') ||
...
const snifferMatch =
  /http((?!http).){26,}?\.(m3u8|mp4|flv|avi|mkv|rm|wmv|mpg)\?.*|http((?!http).){26,}\.(m3u8|mp4|flv|avi|mkv|rm|wmv|mpg)|http((?!http).){26,}\/m3u8\?pt=m3u8.*|http((?!http).)*?default\.ixigua\.com\/.*|http((?!http).)*?cdn-tos[^\?]*|http((?!http).)*?\/obj\/tos[^\?]*|http.*?\/player\/m3u8play\.php\?url=.*|http.*?\/player\/.*?[pP]lay\.php\?url=.*|http.*?\/playlist\/m3u8\/\?vid=.*|http.*?\.php\?type=m3u8&.*|http.*?\/download.aspx\?.*|http.*?\/api\/up_api.php\?.*|https.*?\.66yk\.cn.*|http((?!http).)*?netease\.com\/file\/.*/
  ...
  if (typeExtend !== null) {
    for (let key in typeExtend) {
      if (
        ...
        try {
          str +=
            '筛选' + key + '+全部=+' + typeExtend[key].replace(/,/g, '+') + '\n'
        } catch (e) {}
      }
    }
  }
